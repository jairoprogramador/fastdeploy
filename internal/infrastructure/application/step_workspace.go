...
type FileStepWorkspace struct {
	pathProjectRoot    string
	pathRepositoryRoot string
}

func NewFileStepWorkspace(
	pathProjectRoot,
	pathRepositoryRoot string,
) ports.StepWorkspace {
	return &FileStepWorkspace {
		pathProjectRoot:    pathProjectRoot,
		pathRepositoryRoot: pathRepositoryRoot,
	}
}

func (m *FileStepWorkspace) Prepare(namesRequest dto.NamesParams, runRequest dto.RunParams) (string, error) {
...
func (m *FileStepWorkspace) getPathStepProject(namesRequest dto.NamesParams, runRequest dto.RunParams) string {
...
func (m *FileStepWorkspace) getPathStepRepository(stepName string, repositoryName string) (string, error) {
