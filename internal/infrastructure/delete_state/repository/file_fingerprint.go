...
func (r *FileFingerprintRepository) FindCode(namesRequest appDto.NamesParams, runRequest appDto.RunParams) (*staAgg.FingerprintState, error) {
	fingerprintCode, err := r.findCode(namesRequest, runRequest)
	if err != nil {
		return nil, err
	}

	dto := iStaDto.FingerprintStateDTO{
...
func (r *FileFingerprintRepository) SaveStep(namesRequest appDto.NamesParams, orderRequest appDto.RunParams, fingerprints *staAgg.FingerprintState) error {
	...
	if err := r.saveRecipe(namesRequest, orderRequest, fingerprintDTO.Fingerprints);err != nil {
		return err
	}

	if err := r.saveVars(namesRequest, orderRequest, fingerprintDTO.Fingerprints); err != nil {
		return err
	}
...
func (r *FileFingerprintRepository) SaveCode(namesRequest appDto.NamesParams, runRequest appDto.RunParams, fingerprints *staAgg.FingerprintState) error {
	fingerprintDTO := iStaMap.ToDTO(fingerprints)
	codeFilePath := r.getPathFileStateCode(namesRequest, runRequest)
	return r.saveState(codeFilePath, int(staVos.ScopeCode), fingerprintDTO.Fingerprints)
}

func (r *FileFingerprintRepository) saveRecipe(namesRequest appDto.NamesParams, runRequest appDto.RunParams, fingerprints map[int]string) error {
	recipeFilePath := r.getPathFileStateRecipe(namesRequest, runRequest)
	return r.saveState(recipeFilePath, int(staVos.ScopeRecipe), fingerprints)
}

func (r *FileFingerprintRepository) saveVars(namesRequest appDto.NamesParams, runRequest appDto.RunParams, fingerprints map[int]string) error {
	varsFilePath := r.getPathFileStateVars(namesRequest, runRequest)
	return r.saveState(varsFilePath, int(staVos.ScopeVars), fingerprints)
}

func (r *FileFingerprintRepository) findCode(namesRequest appDto.NamesParams, runRequest appDto.RunParams) (string, error) {
	codeFilePath := r.getPathFileStateCode(namesRequest, runRequest)
	return r.findState(codeFilePath)
}

func (r *FileFingerprintRepository) findRecipe(namesRequest appDto.NamesParams, runRequest appDto.RunParams) (string, error) {
	recipeFilePath := r.getPathFileStateRecipe(namesRequest, runRequest)
	return r.findState(recipeFilePath)
}

func (r *FileFingerprintRepository) findVars(namesRequest appDto.NamesParams, runRequest appDto.RunParams) (string, error) {
	varsFilePath := r.getPathFileStateVars(namesRequest, runRequest)
...
func (r *FileFingerprintRepository) getPathFileStateVars(namesRequest appDto.NamesParams, runRequest appDto.RunParams) string {
	pathStateEnvironment := filepath.Join(r.pathStateRoot, namesRequest.ProjectName(), namesRequest.RepositoryName(), runRequest.Environment())
	return filepath.Join(pathStateEnvironment, fmt.Sprintf("%s.gob", runRequest.StepName()))
}

func (r *FileFingerprintRepository) getPathFileStateCode(namesRequest appDto.NamesParams, runRequest appDto.RunParams) string {
	pathStateProject := filepath.Join(r.pathStateRoot, namesRequest.ProjectName(), namesRequest.RepositoryName(), runRequest.StepName())
	return filepath.Join(pathStateProject, "code.gob")
}

func (r *FileFingerprintRepository) getPathFileStateRecipe(namesRequest appDto.NamesParams, runRequest appDto.RunParams) string {
	pathStateProject := filepath.Join(r.pathStateRoot, namesRequest.ProjectName(), namesRequest.RepositoryName(), runRequest.StepName())
	return filepath.Join(pathStateProject, "recipe.gob")
}
