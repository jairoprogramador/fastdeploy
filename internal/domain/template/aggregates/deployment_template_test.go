...
import (
	...
	"github.com/jairoprogramador/fastdeploy-core/internal/domain/template/entities"
...
func createValidTemplateSource(t *testing.T) shavos.TemplateSource {
	t.Helper()
	source, err := shavos.NewTemplateSource("http://test.com/repo.git", "main")
	if err != nil {
...
func createValidEnvironments(t *testing.T) []depvos.Environment {
	t.Helper()
	env1, err1 := depvos.NewEnvironment(ENV_STAGING, "stag")
	env2, err2 := depvos.NewEnvironment("production", "prod")
	if err1 != nil || err2 != nil {
		t.Fatalf("fallo al crear helpers Environment: %v, %v", err1, err2)
	}
	return []depvos.Environment{env1, env2}
}
...
func createValidSteps(t *testing.T) []entities.StepDefinition {
	...
	verifications2 := []depvos.Trigger{depvos.ScopeVars}
	cmd, err := depvos.NewCommandDefinition("test-cmd", "echo")
	if err != nil {
		t.Fatalf("fallo al crear helper CommandDefinition: %v", err)
	}
	step1, err1 := entities.NewStepDefinition(STEP_TEST, verifications, []depvos.CommandDefinition{cmd}, []depvos.Variable{validVariable})
	step2, err2 := entities.NewStepDefinition("deploy", verifications2, []depvos.CommandDefinition{cmd}, []depvos.Variable{validVariable})
	if err1 != nil || err2 != nil {
...
func TestNewDeploymentTemplate(t *testing.T) {
	...
	envDupe, _ := depvos.NewEnvironment(ENV_STAGING, "stag-dupe")
	dupeEnvs := append(validEnvs, envDupe)

	// Crear un paso duplicado
	cmd, _ := depvos.NewCommandDefinition("cmd", "c")
	stepDupe, _ := entities.NewStepDefinition(STEP_TEST, verifications, []depvos.CommandDefinition{cmd}, []depvos.Variable{validVariable})
...
func TestDeploymentTemplate_DefensiveCopying(t *testing.T) {
	t.Run("Environments debe devolver una copia", func(t *testing.T) {
		...
		modifiedEnv, _ := depvos.NewEnvironment("MODIFIED", "val")
	...
	t.Run("Steps debe devolver una copia", func(t *testing.T) {
		...
		cmd, _ := depvos.NewCommandDefinition("c", "c")
		modifiedStep, _ := entities.NewStepDefinition("MODIFIED", verifications, []depvos.CommandDefinition{cmd}, []depvos.Variable{validVariable})
