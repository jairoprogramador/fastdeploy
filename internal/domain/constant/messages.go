package constant

const MsgInitializeExists = "the project is already initialized"
const MsgInitializeSuccess = "project initialized successfully"

const MsgProjectLoad = "error load project"
const MsgProjectNotFound = "error file project not found"
const MsgProjectNotComplete = "project not complete"
const MsgProjectName = "error getting project name"
const MsgProjectCreating = "error creating project"
const MsgProjectInvalidData = "invalid project data"

const MsgGlobalConfigNotFound = "error file global config not found"
const MsgGlobalConfigCannoBeNull = "error model global config cannot be null"
const MsgGlobalConfigCreating = "error creating global config"


const MessageRunInit = "you must initialize the project, run the command: deploy init"

//const MessageSuccessAddSonarqube = "SonarQube is added successfully"
const MessageSuccessPublish = "service available in: http://localhost:%s/"
const MessageThereAreUnconfirmedChanges = "debe confirmar todos sus cambios antes de publicar"

const MessageErrorNoPackedFile = "error no packed files found"
const MessageErrorCreatingContainer = "error creating container"
const MessageErrorCreatingSonarqube = "error creating sonarqube"
const MessageErrorNoPortHost = "error no host port found"
const MessageErrorInitializingProject = "error initializing project"


